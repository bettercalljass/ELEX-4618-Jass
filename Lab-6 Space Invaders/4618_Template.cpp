...
void analog_test(CControl& board)
{
	...
	while (true)
	{
		...
		bool okX = board.get_data(Analog, JOY_X, chX);
		bool okY = board.get_data(Analog, JOY_Y, chY);
		...
		float xPct = (chX * 100.0f) / 4095.0f;
		float yPct = (chY * 100.0f) / 4095.0f;
...
void digital_test(CControl& board, int buttonPin, int ledPin)
{
	...
	while (true)
	{
		...
		std::cout << "BUTTON = " << !val << '\n';
		board.set_data(Digital, ledPin, !val);
...
void servo_test(CControl& board, int servoChannel)
{
	...
	while (true)
	{
		...
		board.set_data(Servo, servoChannel, (int)pos);
...
void lab3()
{
	...
	while (!board.auto_connect())
	{
		std::cout << "Waiting for microcontroller.\n";
		Sleep(100);
	}
...
void lab4()
{
	CSketch sketch(cv::Size(800, 600), 0);
	sketch.run();
}
