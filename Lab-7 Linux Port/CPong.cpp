...
CPong::CPong()
{
    ...
    screen_w = canvas().cols;
    screen_h = canvas().rows;

    paddle_h = screen_h * 0.20f;
    paddle_w = screen_w * 0.015f;
    ball_radius = screen_h * 0.03f;
...
if (ball_vel.x < 0)
{
    ...
    ai_vel *= damping;
}float max_speed = 250.0f; //difficulty level 150-easy, 250-medium, 350-hard
